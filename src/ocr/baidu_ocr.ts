 ...
 * 需在环境变量配置 `API_KEY`/`SECRET_KEY`（或 `LIEPIN_BAIDU_API_KEY`/`LIEPIN_BAIDU_SECRET_KEY`）。
...
function apiKey(): string | undefined {
  return process.env.API_KEY?.trim() || process.env.LIEPIN_BAIDU_API_KEY?.trim();
}

function secretKey(): string | undefined {
  return process.env.SECRET_KEY?.trim() || process.env.LIEPIN_BAIDU_SECRET_KEY?.trim();
}
...
/** 获取 access_token（带内存缓存，过期前 1 分钟刷新）。 */
export async function getBaiduAccessToken(): Promise<string> {
  const client_id = apiKey();
  const client_secret = secretKey();
  if (!client_id || !client_secret) {
    throw new Error('缺少百度 OCR 凭证：请设置 API_KEY 与 SECRET_KEY（或 LIEPIN_BAIDU_API_KEY / LIEPIN_BAIDU_SECRET_KEY）');
  }
  ...
  if (!res.ok || !data.access_token) {
    throw new Error(`百度 OAuth 失败: ${res.status} ${data.error ?? ''} ${data.error_description ?? ''}`);
  }

  const expiresInSec = typeof data.expires_in === 'number' ? data.expires_in : 2592000;
  cachedToken = { token: data.access_token, expiresAtMs: now + expiresInSec * 1000 };
...
export async function baiduOcrImageBase64(imageBase64: string): Promise<string> {
  ...
  const lines = (data.words_result ?? []).map((r) => (r.words ?? '').trim()).filter((s) => s.length > 0);
  return lines.join('\n').trim();
}
